 ...
 */
function _civigiftaid_civix_civicrm_config(&$config = NULL) {
 ...
 */
function _civigiftaid_civix_civicrm_xmlMenu(&$files) {
  foreach (_civigiftaid_civix_glob(__DIR__ . '/xml/Menu/*.xml') as $file) {
    $files[] = $file;
  }
}
 ...
 */
function _civigiftaid_civix_civicrm_install() {
 ...
 */
function _civigiftaid_civix_civicrm_uninstall() {
  _civigiftaid_civix_civicrm_config();
  if ($upgrader = _civigiftaid_civix_upgrader()) {
 ...
 */
function _civigiftaid_civix_civicrm_upgrade($op, CRM_Queue_Queue $queue = NULL) {
  if ($upgrader = _civigiftaid_civix_upgrader()) {
 ...
 */
function _civigiftaid_civix_civicrm_managed(&$entities) {
  $mgdFiles = _civigiftaid_civix_find_files(__DIR__, '*.mgd.php');
  sort($mgdFiles);
 ...
 */
function _civigiftaid_civix_civicrm_caseTypes(&$caseTypes) {
  if (!is_dir(__DIR__ . '/xml/case')) {
    return;
  }

  foreach (_civigiftaid_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
 ...
 * missing them.
 */
function _civigiftaid_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _civigiftaid_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _civigiftaid_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _civigiftaid_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
 ...
 */
function _civigiftaid_civix_civicrm_alterSettingsFolders(&$metaDataFolders = NULL) {
  static $configured = FALSE;
  if ($configured) {
    return;
  }
